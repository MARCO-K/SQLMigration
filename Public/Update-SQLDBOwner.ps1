...
Function Update-SqlDbOwner
{
  ...
  process { 
    ...
    {
      if($dbname -notin ($server.databases).Name) { 
	  #Write-verbose 'Database does not exist. Skipping dbowner update.'; 
	  continue 
	  }
      ...
      if ($destdb.owner -ne $dbowner)	{
		...
		#Write-verbose 'Database status not normal. Skipping dbowner update.'; 
		continue }
